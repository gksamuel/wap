...
<title>${utilSession.title} - My Pages</title>
    <sql:query var="rs" dataSource="jdbc/workdb">
        select * from pages where seekerid = ? order by id
        <sql:param value="${userSession.id}"/>
    </sql:query>
    <c:if test="${rs.getRowCount() == 0}">
        <jsp:setProperty name="utilSession" property="message" value="Write something interesting about yourself here"/>
    </c:if>
    <p>My Pages</p>
    ...
    <div class="rTable">
        <c:forEach items="${rs.rows}" var="row" varStatus="loop">
            <div class="rTableRow">
                <div class="rTableCell"><a href="edit.jsp?id=${row.id}"><c:out value="${row.title}"/></a></div>
